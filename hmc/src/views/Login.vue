<template>
  <div id="container">
    ...
    <div>
      ...
      <mt-field class="mt_field" type="text" v-model="username"></mt-field>

      <label class="mt_label">密码</label>
      <mt-field class="mt_field" type="password" v-model="password"></mt-field>
      <button class="button" @click="login">登录</button>
      <a href="/registers" class="registers">立即注册</a>
    </div>
    <!-- 表单区域结束 -->
  </div>
</template>
    ...
    login() {
      ...
      this.axios.post("/login", this.qs.stringify(obj)).then(res => {
        ...
        if (res.data.code == 200) {
          //  登录成功
          this.$messagebox.alert("登录成功", "登录提示").then(() => {
            this.$router.push("/");
          });
